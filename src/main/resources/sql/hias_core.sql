...
--Set up Claim Remark History
CREATE TABLE HIAS.CLAIM_REMARK_HISTORY
(
    CLAIM_REMARK_HISTORY_NO INTEGER GENERATED ALWAYS AS IDENTITY,
    CLAIM_NO                INTEGER,
    EMPLOYEE_NO             INTEGER,
    REMARK                  VARCHAR(4000),
    FROM_STATUS_CODE        VARCHAR(3)    DEFAULT 'ACT',
    TO_STATUS_CODE          VARCHAR(3)    DEFAULT 'ACT',
    ACTION_TYPE             VARCHAR(10)   DEFAULT 'ACT',
    IS_DELETED              BOOLEAN       DEFAULT FALSE,
    CREATED_BY              VARCHAR(2000) DEFAULT 'System',
    CREATED_ON              TIMESTAMP     DEFAULT (NOW() AT TIME ZONE 'Asia/Ho_Chi_Minh'),
    MODIFIED_BY             VARCHAR(2000) DEFAULT 'System',
    MODIFIED_ON             TIMESTAMP     DEFAULT (NOW() AT TIME ZONE 'Asia/Ho_Chi_Minh'),
    PRIMARY KEY (CLAIM_REMARK_HISTORY_NO),
    CONSTRAINT FK_CLAIM FOREIGN KEY (CLAIM_NO) REFERENCES HIAS.CLAIM (CLAIM_NO)
);
